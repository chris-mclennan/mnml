...
fn builtin_commands() -> Vec<Command> {
    ...
    let mut cmds = vec![
        ...
        Command {
            id: "editor.section_next_start",
            title: "Jump to next section start (vim `]]`)",
            group: "editor",
            keys: &[],
            run: |app| app.jump_section(true, false),
        },
        Command {
            id: "editor.section_prev_start",
            title: "Jump to previous section start (vim `[[`)",
            group: "editor",
            keys: &[],
            run: |app| app.jump_section(false, false),
        },
        Command {
            id: "editor.section_next_end",
            title: "Jump to end of next section (vim `][`)",
            group: "editor",
            keys: &[],
            run: |app| app.jump_section(true, true),
        },
        Command {
            id: "editor.section_prev_end",
            title: "Jump to end of previous section (vim `[]`)",
            group: "editor",
            keys: &[],
        ...
        // 2026-08-03 — the 27 `forge.open_*` hardcoded launcher
        ...
        // crates.io or a third-party source now ships its own
        // `<id>.open` palette command via `IntegrationManifest.commands`
        // (loaded from `~/.config/mnml/integrations/*.toml`), so
        // duplicating them here would just create palette-name
        // collisions with the manifest-registered dynamic commands.
        //
        // If you're re-adding one, do it through a launcher manifest
        // in the mnml-integrations catalog repo, not here.
        Command {
