...
impl App {
    ...
    pub fn dap_step_in(&mut self) {
        ...
        // a silent no-op. VS Code falls back to the built-in
        // fullscreen toggle for F11 when there's no debug context;
        // mnml routes through `view.fullscreen` when no DAP session
        // is active so muscle memory still lands somewhere useful.
        let Some(mgr) = self.dap.as_mut() else {
