...
fn draw_env_box(
    ...
    // R11 api-workflow SEV-2 (2026-08-23): was `EnvSet::select`,
    // which considered only the runtime override / MNML_ENV /
    // legacy `.rqst/config` — a workspace `[http] default_env`
    // was silently ignored here, so the chip showed `none ▾` +
    // yellow-warning while everywhere else (Vars tab, URL var
    // coloring, actual Send) correctly resolved against the
    // config default. `select_with_config_default` is the same
    // helper draw_request_view uses at line 896 — the chip now
    // agrees with the pane.
    let envset = crate::http::template::EnvSet::select_with_config_default(
        workspace,
        env_override,
        config_default_env,
