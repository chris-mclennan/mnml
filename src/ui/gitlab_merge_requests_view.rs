...
mod tests {
    ...
    #[test]
    fn flatten_mrs_orders_headers_by_config() {
    ...
    #[test]
    fn flatten_my_mrs_empty_cache_is_a_lone_header() {
        let cfg = crate::config::Config::default();
        let dir = tempfile::tempdir().unwrap();
        let app = App::new(dir.path().to_path_buf(), cfg).expect("app new");
        let flat = flatten_my_mrs(&app);
        assert_eq!(flat.len(), 1);
        assert!(matches!(flat[0].kind, RowKind::Header));
        assert_eq!(flat[0].header_label, "mine (cross-project)");
